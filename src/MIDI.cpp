...
collidoscope::MIDIMessage collidoscope::MIDI::parseRtMidiMessage( std::vector<unsigned char> *rtMidiMessage )
{
    ...
    unsigned char mbyte = (*rtMidiMessage)[0];
    // voice is the 4 most significant bits
    unsigned char voice = mbyte >> 4;

    switch ( voice ){
    case 0x9 :
        if (numBytes == 3 && (*rtMidiMessage)[2] == 0) msg.mVoice = MIDIMessage::Voice::eNoteOff;
        else msg.mVoice = MIDIMessage::Voice::eNoteOn;
        break;
    ...
    case 0xB:
            if((*rtMidiMessage)[1] == 58) {
                msg.mChannel = (*rtMidiMessage)[0] & 0x0f;
                msg.mVoice = MIDIMessage::Voice::ePitchBend;
                msg.mData1 = ((*rtMidiMessage)[2] + 6) & 0x7f;
                msg.mData2 = ((*rtMidiMessage)[2] + 6) >> 7;
                return msg;
            }
        else msg.mVoice = MIDIMessage::Voice::eControlChange;
        break;
